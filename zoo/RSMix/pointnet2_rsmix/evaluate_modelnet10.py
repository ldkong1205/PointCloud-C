...
import provider
import modelnet_dataset_origin as modelnet_dataset
# import modelnet_h5_dataset
import modelnet_h5_dataset_origin as modelnet_h5_dataset

parser = argparse.ArgumentParser()
parser.add_argument('--gpu', type=int, default=0, help='GPU to use [default: GPU 0]')
parser.add_argument('--model', default='pointnet2_cls_ssg_origin_modelnet10', help='Model name. [default: pointnet2_cls_ssg]')
...
exp_name = FLAGS.model_path.split('/')[-2]
dump_exp_dir = os.path.join(DUMP_DIR, exp_name)

if not os.path.exists(DUMP_DIR): os.mkdir(DUMP_DIR)
exp_i = 2
l=0
while os.path.exists(dump_exp_dir):
    if l > 0:
        dump_exp_dir = dump_exp_dir[:-7]
    dump_exp_dir = dump_exp_dir+'_eval_'+str(exp_i)
    exp_i += 1
    l += 1
if not os.path.exists(dump_exp_dir): os.mkdir(dump_exp_dir)

LOG_FOUT = open(os.path.join(dump_exp_dir, 'log_evaluate.txt'), 'w')
...
def evaluate(num_votes):
    ...
    with tf.device('/gpu:'+str(GPU_INDEX)):
        ...
        # simple model
        pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, class_num=NUM_CLASSES)
