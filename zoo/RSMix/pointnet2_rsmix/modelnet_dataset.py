...
class ModelNetDataset():
    ...
    def _augment_batch_data(self, batch_data, shuffle=False, jitter=False, rot=False, rdscale=False, shift=False):
        ...
        if rot:
            if self.normal_channel:
                rotated_data = provider.rotate_point_cloud_with_normal(batch_data)
                batch_data = provider.rotate_perturbation_point_cloud_with_normal(rotated_data)
            else:
                rotated_data = provider.rotate_point_cloud(batch_data)
                batch_data = provider.rotate_perturbation_point_cloud(rotated_data)
        if rdscale:
    ...
    def _rsmix_batch_data(self, batch_data, label_batch, beta=0.0, n_sample=512):
        return rsmix_provider.rsmix(batch_data, label_batch, beta=beta, n_sample=512)
    
    def _get_item(self, index): # load 1 data.
    ...
    def next_batch(self, augment=False, convda=False, rddrop=False, rsmix_prob=0.5, beta=0.0, 
        ...
        revised from
        '''
        lam = np.zeros(batch_data.shape[0],dtype=float)
        batch_label_b = batch_label
        if augment: 
            r = np.random.rand(1)
            # r = 0.1 # for debug
            ...
            if beta > 0 and r < rsmix_prob:
                batch_data, lam, batch_label, batch_label_b = self._rsmix_batch_data(batch_data, batch_label, beta=beta, n_sample=512)
        '''
        to here
        '''
        return batch_data, batch_label, lam, batch_label_b
