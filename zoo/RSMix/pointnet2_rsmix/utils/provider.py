...
def rotate_point_cloud_with_normal_for_part_seg(batch_xyz_normal):
    ''' Randomly rotate XYZ, normal point cloud.
        Input:
            batch_xyz_normal: B,N,6, first three channels are XYZ, last 3 all normal
        Output:
    ...
    for k in range(batch_xyz_normal.shape[0]):
        ...
        shape_pc = batch_xyz_normal[k,:,0:3]
        shape_normal = batch_xyz_normal[k,:,3:6]
        seg_label = batch_xyz_normal[k,:,6]
        batch_xyz_normal[k,:,0:3] = np.dot(shape_pc.reshape((-1, 3)), rotation_matrix)
        batch_xyz_normal[k,:,3:6] = np.dot(shape_normal.reshape((-1, 3)), rotation_matrix)
        batch_xyz_normal[k,:,6] = seg_label
    return batch_xyz_normal

def rotate_perturbation_point_cloud_with_normal_for_part_seg(batch_data, angle_sigma=0.06, angle_clip=0.18):
    """ Randomly perturb the point clouds by small rotations
        Input:
          BxNx6 array, original batch of point clouds and point normals
        Return:
    ...
    for k in list(range(batch_data.shape[0])):
        ...
        shape_pc = batch_data[k,:,0:3]
        shape_normal = batch_data[k,:,3:6]
        seg_label = batch_data[k,:,6]
        rotated_data[k,:,0:3] = np.dot(shape_pc.reshape((-1, 3)), R)
        rotated_data[k,:,3:6] = np.dot(shape_normal.reshape((-1, 3)), R)
        rotated_data[k,:,6] = seg_label
    return rotated_data
##################
