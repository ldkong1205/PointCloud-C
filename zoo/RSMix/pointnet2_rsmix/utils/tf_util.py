...
def _variable_with_weight_decay(name, shape, stddev, wd, use_xavier=True):
  ...
  # for tensor >=2.0
  # if use_xavier:
  #   var = tf.Variable(tf2.initializers.GlorotUniform()(shape=shape))
  # else:
  #   initializer = tf.truncated_normal_initializer(stddev=stddev)
  #   var = _variable_on_cpu(name, shape, initializer)
...
def batch_norm_template(inputs, is_training, scope, moments_dims_unused, bn_decay, data_format='NHWC'):
  ...
  return tf.contrib.layers.batch_norm(inputs, 
                                      ...
                                      scope=scope,
                                      data_format=data_format)
  # return tf2.keras.layers.BatchNormalization(momentum=bn_decay, 
                                            #  center=True, scale=True)(inputs, training=is_training)
  # return keras.layers.BatchNormalization(momentum=bn_decay, 
                                            #  center=True, scale=True)(inputs, training=is_training)
  # return tf.layers.batch_normalization(inputs, center=True, scale=True, training=is_training, momentum=bn_decay)
