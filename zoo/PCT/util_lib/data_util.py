...
def load_data(partition, corruption_type, level):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, '../data')
    all_data = []
    all_label = []
    if corruption_type in ['clean', 'composite']:
        h5_name = os.path.join(DATA_DIR, 'modelnet40_c', '{}.h5'.format(corruption_type))
    else:
        h5_name = os.path.join(DATA_DIR, 'modelnet40_c', '{}_{}.h5'.format(corruption_type, level))
    f = h5py.File(h5_name)
...
class ModelNet40(Dataset):
    def __init__(self, args, corruption_type, level, partition='train'):
        self.data, self.label = load_data(partition, corruption_type, level)
        self.num_points = args.num_points
        self.partition = partition

    def __getitem__(self, item):
        # pointcloud = self.data[item][:self.num_points] #(1024,3)
        pointcloud = self.data[item]  # (1024,3)
